...
local function RemoveUsedIngredients(recipe, t)
	for ingredient, requiredAmount in pairs( t.Recipe_Ingredients ) do
		if ( ingredient ) then
			self.Ingredients[ingredient].Amount = self.Ingredients[ingredient].Amount - requiredAmount
		else
...
net.Receive( "DarkRP_StoveSpawnFood", function (len, ply)
	...
	print ( "HEY FUCKFACE", recipeEnt )
	print ( "HEY GALUNGA", table.ToString( recipeTable, false ) )
	
	entself.Sparking = true
	RemoveUsedIngredients(recipeEnt, recipeTable)
